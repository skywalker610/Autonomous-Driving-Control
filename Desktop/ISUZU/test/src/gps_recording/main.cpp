	...
	*/
	...
	while(ros::ok()){
		...
		if(i==1){
			...
			ofstream write_relative_pos;
			write_relative_pos.open("rtk_waypoints_test.dat");
			write_relative_pos.close();
		}
		...
		if(wps_step >= 0.5){
			...
			// Change the gps position to the center of gravity
			x0 = x0 + lr*cos(hedg);
			y0 = y0 + lr*sin(hedg);
			ofstream write_relative_pos;
			write_relative_pos.open("rtk_waypoints_test.dat", ios::app); // adding ios:app will write data below the last line
			write_lat << std::fixed << std::setprecision(7) << x0<<","<<y0	 << endl;
			write_lat.close();
		}
