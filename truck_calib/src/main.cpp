...
int main ( int argc, char**argv){
	...
	// x0 = (lon - lon_ref)*111132.944444;
	// y0 = (lat - lat_ref)*82230.670304;
	...
	*/
	...
	double steer_wheel_angle = 720; // in degree, this is the steer wheel angle??????????????????????????????
	double delta = 40 * 3.1415926 / 180 *steer_wheel_angle/900; // this is the steering angle of the front tire ?????????????????????????????

	// plot predicted trajectory by vehicle model ========================================================
	...
	*/
	...
	double traj_x[200], traj_y[200];
	double v_const = 3;  // 3 m/iiiiiiiiiiiiiiiiiiui s
	double dt = 0.2;  // time step in second
	double Lf = 2.76; // in meter, This is the length from front to CoG that has a similar radius.
	...
	for(int j = 0; j < 20; j++){
		traj_x[j+1] = traj_x[j] + v_const * cos( hedg ) * dt;
		traj_y[j+1] = traj_y[j] + v_const * sin( hedg ) * dt;
		printf("heading: %f;\n",hedg);
		// exit(0);
		hedg = hedg + v_const / Lf * delta * dt;
	...
	while(ros::ok()){
		...
		// publish predicted trajectory to rviz
		// if(i<50){
		...
		if(i >= 1){
			...
			points.scale.x = 1;// %Tag(SCALE)% POINTS markers use x and y scale for width/height respectively
			// points.scale.y = 2;
